...
int VDI_DrawBars_vrocpyfm(framebuffer_t *framebuffer, fbrect_t *rect)
{
	MFDB src_mfdb, dst_mfdb;
	short blitcoords[8], x1, y1, x2, y2;

	if (framebuffer->buffer==NULL) {
		fprintf(stderr, "No temporary buffer allocated\n");
		return -1;
	}
	...
	if ((x1==x2) || (y1==y2)) {
		fprintf(stderr, "No rectangle to copy\n");
		return -1;
	}
...
void VDI_DrawBars_vbar(framebuffer_t *framebuffer, fbrect_t *rect, int component)
{
	...
	for (i=0; i<rect->w;i++) {
		...
		pxy[1] = rect->y + 0;
		pxy[2] = pxy[0] + 0;
		pxy[3] = pxy[1] + rect->h - 1;
		v_bar(vdi_handle,pxy);
	}
}

void VDI_DrawRect(framebuffer_t *framebuffer, fbrect_t *rect, int component)
{
	short rgb[3], pxy[16];
	int x, numshades;
	...
	if (framebuffer->bpp > 8) {
		/* Set color to max value */
		rgb[0] = rgb[1] = rgb[2] = 0;
		switch(component) {
			case COMPONENT_RED: rgb[0] = 1000; break;
			case COMPONENT_GREEN: rgb[1] = 1000; break;
			case COMPONENT_BLUE: rgb[2] = 1000; break;
		}
		vs_color(vdi_handle, vdi_index[x], rgb);
	}
	vsl_color(vdi_handle, vdi_index[x]);

	/* Draw a cross + non-filled rectangle */
	pxy[0] = rect->x;
	pxy[1] = rect->y;
	pxy[2] = pxy[0] + rect->w - 1;
	pxy[3] = pxy[1];
	pxy[4] = pxy[2];
	pxy[5] = pxy[1] + rect->h - 1;
	pxy[6] = pxy[0];
	pxy[7] = pxy[5];
	pxy[8] = pxy[0];
	pxy[9] = pxy[1];
