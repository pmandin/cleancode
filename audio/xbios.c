...
int main(int argc, char **argv)
{
	...
	if (cookie_snd & SND_16BIT) fprintf(output_handle, " 16 bits CODEC\n");
	if (cookie_snd & SND_DSP) fprintf(output_handle, " DSP 56k\n");
	if (cookie_snd & SND_MATRIX) fprintf(output_handle, " Connection matrix\n");
	if (cookie_snd & SND_EXT) fprintf(output_handle, " GSXB audio\n");
	...
	/* Available with MagicSound driver */
